...
print_fg_color_charge() {
	percentage=$($CURRENT_DIR/battery_percentage.sh | sed -e 's/%//')
	if [ $percentage -ge 95 -o "$percentage" == "" ]; then
		# if percentage is empty, assume it's a desktop
		printf "$color_charge_primary_tier8"
	elif [ $percentage -ge 80 ]; then
		printf "$color_charge_primary_tier7"
	elif [ $percentage -ge 65 ]; then
		printf "$color_charge_primary_tier6"
	elif [ $percentage -ge 50 ]; then
		printf "$color_charge_primary_tier5"
	elif [ $percentage -ge 35 ]; then
		printf "$color_charge_primary_tier4"
	elif [ $percentage -ge 20 ]; then
		printf "$color_charge_primary_tier3"
	elif [ $percentage -gt 5 ]; then
		printf "$color_charge_primary_tier2"
	else
		printf "$color_charge_primary_tier1"
	fi
}

print_bg_color_charge() {
	percentage=$($CURRENT_DIR/battery_percentage.sh | sed -e 's/%//')
	if [ $percentage -ge 95 -o "$percentage" == "" ]; then
		# if percentage is empty, assume it's a desktop
		printf "$color_charge_secondary_tier8"
	elif [ $percentage -ge 80 ]; then
		printf "$color_charge_secondary_tier7"
	elif [ $percentage -ge 65 ]; then
		printf "$color_charge_secondary_tier6"
	elif [ $percentage -ge 50 ]; then
		printf "$color_charge_secondary_tier5"
	elif [ $percentage -ge 35 ]; then
		printf "$color_charge_secondary_tier4"
	elif [ $percentage -ge 20 ]; then
		printf "$color_charge_secondary_tier3"
	elif [ $percentage -gt 5 ]; then
		printf "$color_charge_secondary_tier2"
	else
		printf "$color_charge_secondary_tier1"
	fi
}

print_color_charge() {
	local primary_plane="$1"
	if [ "$primary_plane" == "fg" ]; then
		print_fg_color_charge
	else
		print_bg_color_charge
	fi
